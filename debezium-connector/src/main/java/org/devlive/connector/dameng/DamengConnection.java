...
public class DamengConnection extends JdbcConnection {
    ...
    private final static Logger LOGGER = LoggerFactory.getLogger(DamengConnection.class);
    ...
    public DamengConnection(JdbcConfiguration config, ConnectionFactory connectionFactory, boolean showVersion) {
        ...
        this.databaseVersion = resolveDamengDatabaseVersion();
        if (showVersion) {
    ...
    public DamengDatabaseVersion getDamengVersion() {
        return databaseVersion;
    }

    private DamengDatabaseVersion resolveDamengDatabaseVersion() {
        String versionStr;
        try {
            try {
                // Dameng 18.1 introduced BANNER_FULL as the new column rather than BANNER
                // This column uses a different format than the legacy BANNER.
                versionStr = queryAndMap("SELECT BANNER_FULL FROM V$VERSION WHERE BANNER_FULL LIKE 'Dameng Database%'", (rs) -> {
                    if (rs.next()) {
            ...
            if (versionStr == null) {
                versionStr = queryAndMap("SELECT BANNER FROM V$VERSION WHERE BANNER LIKE 'Dameng Database%'", (rs) -> {
                    if (rs.next()) {
                        return rs.getString(1);
                    }
                    return null;
            ...
            if (e instanceof SQLRecoverableException) {
                throw new RetriableException("Failed to resolve Dameng database version", e);
            }
            throw new RuntimeException("Failed to resolve Dameng database version", e);
        }

        if (versionStr == null) {
            throw new RuntimeException("Failed to resolve Dameng database version");
        }
    ...
    protected String resolveCatalogName(String catalogName) {
        final String pdbName = DamengUtils.getObjectName(config().getString("pdb.name"));
        final String databaseName = DamengUtils.getObjectName(config().getString("dbname"));
        return !DamengUtils.isObjectNameNullOrEmpty(pdbName) ? pdbName : databaseName;
    }
    ...
    public boolean validateLogPosition(Partition partition, OffsetContext offset, CommonConnectorConfig config) {
        final DamengConnectorConfig connectorConfig = (DamengConnectorConfig) config;
        ...
        final Scn storedOffset = ((DamengConnectorConfig) config).getAdapter().getOffsetScn((DamengOffsetContext) offset);
    ...
    public String buildSelectWithRowLimits(TableId tableId,
        ...
        if (getDamengVersion().getMajor() < 12) {
            sql
    ...
    public static String connectionString(JdbcConfiguration config) {
        return config.getString(URL) != null ? config.getString(URL)
                : DamengConnectorConfig.ConnectorAdapter.parse(config.getString("connection.adapter")).getConnectionUrl();
    }
    ...
    protected ColumnEditor overrideColumn(ColumnEditor column) {
        ...
        if (DamengTypes.TIMESTAMP == column.jdbcType()) {
            column.length(column.scale().orElse(Column.UNSET_INT_VALUE)).scale(null);
        }
        else if (DamengTypes.NUMBER == column.jdbcType()) {
    ...
    protected Map<TableId, List<Attribute>> getAttributeDetails(TableId tableId, String tableType) {
        ...
        try {
            getDatabaseObjectDetails(tableId, tableType, (objectId, dataObjectId) -> {
                ...
                attributes.add(Attribute.editor().name(DamengDatabaseSchema.ATTRIBUTE_OBJECT_ID).value(objectId).create());
                attributes.add(Attribute.editor().name(DamengDatabaseSchema.ATTRIBUTE_DATA_OBJECT_ID).value(dataObjectId).create());
            ...
            return queryAndMap(query, rs -> {
                ...
                while (rs.next()) {
                    ...
                    if (!rs.wasNull()) {
                        ...
                        if (getDamengVersion().getMajor() >= 11) {
